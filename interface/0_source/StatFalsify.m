...
%%
 ...
 %%  
    function [list,falsified,nb,falsi_point] = PseudoRandSamp(this,CBS,param_names,...
   ...
   function this = StatFalsify(obj,CBS,phi,rob_wt,init_samples,max_simulations,max_time)                              
     ...
     % MAX_TIME: Limit on computation time.
      ...
      global start
        ...
        % parameter space.
            [this.clusters{1},falsified,nb_added,this.falsifier] =...
        ...
        run = (time<max_time) && num_sim<max_simulations && ~falsified ;        
        while run && start == 1
        disp('num_sim =')   
        disp(num_sim)
...
%                          max(1,sum(log((bounds(:,2)-bounds(:,1))./eps+1)));
       ...
       % and region containing falsifier.
        ...
        % Perform probability based sampling
           ...
           [s,falsified,nb_added,this.falsifier] =... 
         ...
         run = (time<max_time) && num_sim<max_simulations && ~falsified ; 
             ...
             for i = 1:numel(this.regions)
                 if ~isempty(this.clusters{i}.pts)
                  ...
                  this.lower_bounds.pts = [];
                  this.lower_bounds.vals = inf;
                 end                
             end
